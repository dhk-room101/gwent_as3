...
package scaleform.clik.managers {
	...
	// Internal	
	    internal var _inputEventDisabled:Boolean=false;
		internal var _swapAcceptCancel:Boolean=false;
    ...
    // Initialization:
		...
		public function set swapAcceptCancel(request:Boolean):void
		{
			this._swapAcceptCancel = request;
			return;
		}
    ...
    // Public Methods:
		...
		public function disableInputEvents(inputEventDisabled:Boolean):void
        {
            this._inputEventDisabled = inputEventDisabled;
            if (stage) 
            {
    ...
    // Protected Methods:
		...
		protected function handleGamePad(gamePadAnalogEvent:GamePadAnalogEvent):void
		{
			var inputDetails:InputDetails = null;
			var keyCode:int = 0;
			var stickUsed:Boolean = false;
			var x:Number = gamePadAnalogEvent.xvalue;
			var y:Number = gamePadAnalogEvent.yvalue;
			var code:uint = gamePadAnalogEvent.code;
			switch (code) 
			{
				case GamePad.PAD_LT:
				{
					keyCode = KeyCode.PAD_LEFT_STICK_AXIS;
					stickUsed = true;
					break;
				}
				case GamePad.PAD_RT:
				{
					keyCode = KeyCode.PAD_RIGHT_STICK_AXIS;
					stickUsed = true;
					break;
				}
				case GamePad.PAD_L2:
				{
					keyCode = KeyCode.PAD_LEFT_TRIGGER_AXIS;
					break;
				}
				case GamePad.PAD_R2:
				{
					keyCode = KeyCode.PAD_RIGHT_TRIGGER_AXIS;
					break;
				}
				default:
				{
			...
			if (keyCode != 0) 
			{
				inputDetails = new InputDetails("axis", keyCode, new InputAxisData(x, y), null, gamePadAnalogEvent.controllerIdx, false, false, false, stickUsed);
				dispatchEvent(new InputEvent(InputEvent.INPUT, inputDetails));
			}
			return;
		}
    }
